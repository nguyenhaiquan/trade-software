...
namespace stocksRanking
{
    public class StocksRanking
    {
        ...
        public void RankingOfADateAllStocks(DateTime date)
        {
            //1.Lay tat ca stock trong database
            DbSet<databaseEntity.stockCode> stockCodeList;
            GetAllStockCodes(out stockCodeList);

            //2. Lay strategy Ranking 
            string StrategyCode = "";

            for (int rowId = 0; rowId < stockCodeList.Count<stockCode>(); rowId++)
            {
